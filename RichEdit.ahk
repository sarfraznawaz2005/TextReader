...
Class RichEdit {
   ...
   __Delete() {
      try {
         If IsObject(This.RE) && DllCall("IsWindow", "Ptr", This.RE.Hwnd) {
            If (RichEdit.SubclassCB) {
               DllCall("RemoveWindowSubclass", "Ptr", This.RE.Hwnd, "Ptr", RichEdit.SubclassCB, "Ptr", 0)
            }
         }
      } catch {
         ; Control is already destroyed, do nothing
      }
      This.RE := 0
   }
   ; ===================================================================================================================
   ...
   ; -------------------------------------------------------------------------------------------------------------------
   SetReadOnly(State) { ; Sets or removes the READONLY option
      ; State: True to set, False to remove.
      readOnlyFlag := 0x800 ; ECO.READONLY
      currentOptsVal := SendMessage(0x044E, 0, 0, This.HWND) ; EM_GETOPTIONS
      if (State) {
         newOptsVal := currentOptsVal | readOnlyFlag
      } else {
         newOptsVal := currentOptsVal & ~readOnlyFlag
      }
      Return SendMessage(0x044D, 1, newOptsVal, This.HWND) ; EM_SETOPTIONS with ECOOP_SET
   }
   ; -------------------------------------------------------------------------------------------------------------------
   ...
   LoadRTF(FilePath, Selection := False) { ; Loads RTF file into the control
      ...
      If !(fileObj := FileOpen(FilePath, "r"))
         Return False
      ES := Buffer((A_PtrSize * 2) + 4, 0)                     ; EDITSTREAM structure
      NumPut("UPtr", fileObj.Handle, ES)                          ; dwCookie
   ...
   FindText(Find, Mode?) { ; Finds Unicode text within a rich edit control
      ...
      Sel := This.GetSel()
      minChar := (Flags & FR.DOWN) ? Sel.E : Sel.S
      maxChar := (Flags & FR.DOWN) ? -1 : 0
      FTX := Buffer(16 + A_PtrSize, 0)
      NumPut("Int", minChar, "Int", maxChar, "UPtr", StrPtr(Find), FTX)
      SendMessage(0x047C, Flags, FTX.Ptr, This.HWND)
      startChar := NumGet(FTX, 8 + A_PtrSize, "Int"), endChar := NumGet(FTX, 12 + A_PtrSize, "Int")
      If (startChar = -1) && (endChar = -1)
         Return False
      This.SetSel(startChar, endChar)
      This.ScrollCaret()
      Return True
   }
   ; -------------------------------------------------------------------------------------------------------------------
   ...
   ClearAllFormatting() {
      ...
      CF2 := RichEdit.CHARFORMAT2()
      CF2.Mask := 0xFFFFFFFF ; Set all masks to ensure all properties are considered
      CF2.Effects := 0       ; Clear all effects (bold, italic, etc.)
      CF2.Height := This.DefFont.Size * 20 ; Default font size in twips
      CF2.TextColor := DllCall("GetSysColor", "Int", 8, "UInt") ; COLOR_WINDOWTEXT (default text color)
      CF2.BackColor := DllCall("GetSysColor", "Int", 5, "UInt") ; COLOR_WINDOW (default background color)
      StrPut(This.DefFont.Name, CF2.Ptr + 26, 32) ; Default font name
      SendMessage(0x0444, 1, CF2.Ptr, This.HWND) ; EM_SETCHARFORMAT

      ; Reset paragraph formatting
      PF2 := RichEdit.PARAFORMAT2()
      PF2.Mask := 0xFFFFFFFF ; Set all masks to ensure all properties are considered
      PF2.Alignment := 1     ; PFA_LEFT (Left alignment)
      PF2.StartIndent := 0
      PF2.RightIndent := 0
      PF2.Offset := 0
      PF2.Numbering := 0     ; No numbering
      PF2.SpaceBefore := 0
      PF2.SpaceAfter := 0
      PF2.LineSpacing := 20  ; Single spacing (20 twips per line)
      PF2.LineSpacingRule := 5 ; Single spacing rule
      ; Clear tab stops
      PF2.Tabs := [] ; Clear the array of tabs
      SendMessage(0x0447, 0, PF2.Ptr, This.HWND) ; EM_SETPARAFORMAT
      Return True
   }
   ; -------------------------------------------------------------------------------------------------------------------
   ...
   ; -------------------------------------------------------------------------------------------------------------------
   GetLine(LineNumber) { ; Get the text of a specific line
      ; LineNumber: 1-based line number
      If (LineNumber < 1)
         Return ""
      
      lineIndex := This.GetLineIndex(LineNumber - 1)
      if (lineIndex = -1)
         Return ""
      lineLength := SendMessage(0xC1, lineIndex, 0, This.Hwnd) ; EM_LINELENGTH
      if (lineLength = 0)
         Return ""

      buf := Buffer(lineLength * 2 + 2, 0)
      NumPut("UShort", lineLength, buf)
      SendMessage(0xC4, LineNumber - 1, buf.Ptr, This.Hwnd) ; EM_GETLINE
      Return StrGet(buf.Ptr, "UTF-16")
   }
   ; -------------------------------------------------------------------------------------------------------------------
   GetLineFromChar(CharIndex) { ; Get the line number from a character index
      ; EM_EXLINEFROMCHAR = 0x0436
      Return SendMessage(0x0436, 0, CharIndex, This.Hwnd) + 1
   }
   ; ===================================================================================================================
   ; Statistics
   ; ===================================================================================================================
